...
final class Validator implements ValidatorInterface
{
     ...
     *
     * Priority order:
     * 1. Already a Rule object - return directly
     * 2. RuleManager (includes built-in Rule classes and custom rules)
     * 3. Validator's custom callable rules (legacy support)
     * 4. Validator's built-in validateXxx() methods (legacy fallback)
     *
     * @param string|RuleInterface $rule Rule string or Rule object
     * @return RuleInterface|string Rule object or string for legacy built-in rules
     */
    private function resolveRule(string|RuleInterface $rule): RuleInterface|string
    {
        ...
        // Try RuleManager first (includes built-in Rule classes)
        ...
        // Check if it's a Validator custom rule (callable) - legacy support
        if (isset(self::$customRules[$ruleName])) {
            return $rule;
        }

        // Fall back to built-in validateXxx() method if exists - legacy support
        $method = 'validate' . str_replace('_', '', ucwords($ruleName, '_'));
        if (method_exists($this, $method)) {
            return $rule;
        }

        // Unknown rule - will throw in validateRule()
        return $rule;
    }
    ...
    // =========================================================================
    // Database Validation Rules (Legacy - kept for backward compatibility)
    // =========================================================================
