 ...
 *
 * Usage:
 *   new RequiredUnless('payment_type', 'cash')
 *   new RequiredUnless('role', ['admin', 'moderator'])
 *
 ...
 */
final class RequiredUnless implements DataAwareRuleInterface, ImplicitRuleInterface
{
    private ?ValidationData $data = null;

    /**
     * @var array<mixed> Values that exempt the required condition
     */
    private readonly array $values;

    /**
     * @param string $field The field to check
     * @param mixed $values Value(s) that exempt this field from being required
     */
    ...
    public function passes(string $attribute, mixed $value): bool
    {
        ...
        // Exemption not met - field is required
    ...
    private function exemptionMet(): bool
    {
        ...
        foreach ($this->values as $exemptValue) {
            // Handle boolean comparisons
            if (is_bool($exemptValue)) {
                if ($this->toBool($otherValue) === $exemptValue) {
                    return true;
                }
                continue;
            }
    ...
    public function message(): string
    {
        ...
        return "The :attribute field is required unless {$this->field} is {$valuesStr}.";
    }
}
